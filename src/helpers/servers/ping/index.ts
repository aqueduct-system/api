    ...
    return new Promise((resolve, reject) => {
        const socket = createConnection(
            ...
            async () => {
                ...
                const handshakeData = await packetDecoder.oncePromise<
                    IHandshakeData
                >("handshake");
                // ping
                socket.write(createPingPacket(BigInt(Date.now())));
                const pingData = await packetDecoder.oncePromise<number>(
                    "pong"
                );
                if (timeout) {
                    clearTimeout(timeout);
...
const createPacket = (packetId: number, data: Buffer): Buffer => {
    return Buffer.concat([
        Buffer.from(encode(encodingLength(packetId) + data.length)),
